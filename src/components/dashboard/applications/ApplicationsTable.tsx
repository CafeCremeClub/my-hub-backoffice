...
import ApplicationsTablePaginationControls from '@/components/dashboard/applications/ApplicationsTablePaginationControls';
...
const ApplicationsTable = ({
  ...
  return (
    <>
      ...
      <div
        className={`flex flex-col gap-4 border border-[#EAECF0] rounded-[0.75rem] ${
          applications.length === 0 ? 'h-full' : ''
        }`}
      >
        <div className="flex items-center gap-2 px-4 pt-4 text-[#101828] font-semibold text-2xl  bricolage-grotesque tracking-tighter">
          Les candidatures à ce poste
          <Badge className="bg-[#ECFDF3] text-[#079455] text-xs font-medium min-w-10 h-6 rounded-full border border-[#ABEFC6]">
            {count}
          </Badge>
        </div>
        {applications && applications.length > 0 ? (
          <Table>
            <TableHeader className="bg-[#F4F9FF] h-11 border-t border-t-[#EAECF0]">
              <TableRow>
                <TableHead className="text-xs font-medium text-[#475467] min-w-72">
                  Nom
                </TableHead>
                ...
                >
                  <TableCell className="font-medium text-[#101828] text-sm">
                    <div className="flex items-center gap-2">
                      <div className="flex items-center justify-center bg-[#F2F4F7] border border-[#00000014] size-10 rounded-full flex-none text-[#667085] font-semibold bricolage-grotesque tracking-tighter">
          ...
          <NoDataBox message="Aucune application disponible pour le moment." />
        )}
