  ...
  const simulateUpload = (file: File) => {
    ...
    const interval = setInterval(() => {
      setUploadedFile((prev) => {
        ...
        const increment =
          prev.progress < 90 ? Math.random() * 15 + 5 : 100 - prev.progress;
      ...
      {uploadedFile ? (
        ...
        >
          <div className="flex flex-col items-center gap-5">
            <div className="flex justify-center items-center size-10 rounded-[0.625rem] border shadow shadow-[#1018280D] border-[#EAECF0]">
              <LuCloudUpload className="size-5 text-[#344054]" />
            </div>
            <div className="flex flex-col items-center text-sm text-[#475467]">
              <div>
                <span className="text-[#0040C1] font-semibold">
                  Télécharger le fichier
                </span>{' '}
                <span>ou glisser et déposer</span>
              </div>
              <p>{acceptedFormats.join(', ')} (max. 800x400px)</p>
            </div>
          </div>
