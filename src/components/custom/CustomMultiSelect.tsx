    ...
    <div className="relative">
      ...
      >
        <div
          ref={containerRef}
          className={cn(
            'shadow-sm shadow-[#1018280D] px-3.5 !min-h-[2.75rem] !h-auto bg-white border text-sm focus:outline-none focus:!ring-0 transition-all duration-200 w-full text-left flex items-center justify-between cursor-pointer',
            getBorderRadius(),
            isError
        ...
        {/* Hidden SelectTrigger for accessibility */}
        <SelectTrigger className="sr-only" disabled={disabled}>
          <SelectValue />
        </SelectTrigger>

        <SelectContent className="max-h-60 w-full -mt-8">
          {availableOptions.length === 0 ? (
            <div className="px-3 py-2 text-[#667085] text-sm">
            ...
            availableOptions.map((option) => (
              <SelectItem
                key={option.value}
                value={option.value}
                className="cursor-pointer text-sm text-[#1B55F5]"
              >
                {option.label}
              </SelectItem>
            ))
